...
movedone = 0    #move legality thread is finished
spacedone = 0   #move blocking thread is finished (there is space)
checkdone = 0   #check/mate thread is finished
...
def BishopMove(rank, file, index):
	plist = []
	edge = [0,8,16,24,52,40,48,56,7,15,23,31,39,47,55,63]
	# only include squares that are on the board.
	if(file == 0):
		diff = 9
		while(index+diff < 64):
			plist.append(index+diff)
			if(index+diff in edge):
				diff = 64 #break
			diff = diff + 9
		while(index+diff > -1):
			plist.append(index+diff)
...
#checks if there are other pieces blocking the move
#spaceThread updates the boardstate each move.
class spaceThread(threading.Thread):
	def run(self):
		global im
		global fail
		global done
		...
		for m in movelist:
			...
			if(" " in m):	
				...
				if(wm == "O-O"):
					...
					#sidlists and plists are generateed by move thread.
					#this one checks the spaces and produces newList2
					# the intersection of newList and newList2 should be exactly 1 square.							
					...
					#now in the barrier, I use data from moveThread
			...
			if(len(newList) != 1):
				...
				mutex.release()
				print("Board Updated wm: piece from ",newList[0] , " moved to ", index ,".")
			wm_barrier.reset()
			...
			if(len(newList) != 1):
				...
				if(newList[0] > 63 or newList[0] < 0):
					...
					boardstate[newList[0]] = '0'
				mutex.release()
				print("Board Updated bm: piece from ",newList[0] , " moved to ", index ,".")
			bm_barrier.reset()
